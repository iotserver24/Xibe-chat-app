...
class ChatProvider extends ChangeNotifier {
  ...
  // Expose database service so main.dart can set user ID
  ...
  // Public method to reload chats (used after cloud sync)
  Future<void> reloadChats() async {
    await _loadChats();
  }
  
  // Reload current chat messages from database
  Future<void> reloadCurrentChat() async {
    if (_currentChat == null || _currentChat!.id == null) return;
    ...
    try {
      ...
      _messages = await _databaseService.getMessagesForChat(_currentChat!.id!, limit: 200);
      _error = null;
    } catch (e) {
      _error = 'Failed to reload chat: $e';
    } finally {
      _isLoading = false;
      notifyListeners();
    }
  ...
  Future<void> selectChat(Chat chat) async {
    _currentChat = chat;
    // Load only recent messages (200) to prevent memory issues
    // For large chats, this significantly reduces RAM usage
    _messages = await _databaseService.getMessagesForChat(chat.id!, limit: 200);
    _error = null;
    notifyListeners();
  }
